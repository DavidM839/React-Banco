  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Procesa los datos para calcular intervalos de tiempo y montos de retiros.
        const intervalos = {};
        ...
        const intervaloData = Object.keys(intervalos).map(mes => ({ mes, retiro: intervalos[mes] }));
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data}>
          ...
          <Tooltip />
          <Legend />
          <Bar dataKey="retiro" fill="red" name="Retiros" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
